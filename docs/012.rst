...
Newsletter #12 / December 23, 1993

CONTENTS

   * HDF3.3 Release 2
   * HDF3.3r2 bug fixes and support for Solaris2.0
   * HDF3.3 Reference Manual available
   * ANSI C in HDF 4.0
   * Proposed N-bit Dataset Routines
   * HDF Help Desk and HDF Information Server
   * Update on Collage, Mosaic, etc
   * New Contributions
...
141.142.20.50) in the directory HDF/HDF3.3r2/. File ABOUT_3.3r2 and
README files in that directory and subdirectories contain full details
on the new features of HDF 3.3, as well as directions on how to
...
in HDF/HDF3.3r2/patches/. For the convenience of users who don't have
easy access to patch facilities, we put all patched source code in
HDF/HDF3.3r2/patches/patchedsrc/.
...
A complete draft of the "HDF 3.3. Reference Manual, Draft Version 3.3"
...
The NCSA ftp directory Documentation/HDF3.3/Ref_Manual contains
postscript files that constitute the manual. See the README in the
directory for further information.

If you need hardcopy, contact the office of NCSA Documentation Orders
at:
...
store datasets which are composed of number types which don't fall into
any of our current pre-defined number types. This type of data could
be 12-bit integers, 15-bit floating point numbers, and possibly
...
multi-file SDS and the VSet routines. The following discussion relates
only to the SDS extensions, VSet extensions for bitfields will be
handled at a later date.

Writing N-Bit datasets
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
This server will perhaps become a permanent part of HDF. We would like
to hear your feedback about this system.
...
weeks. XCollage 1.2.1 now supports HDF3.3. Mosaic is now also
...
information about NCSA software, see NCSA's Information Server or
documentation on the ftp server.
...
------------------------------------------------------------------------
   ...
   * fix version tag
     ...
     Fixver.c in subdirectory fixver/ is contributed by Doug Ilg,
     Hughes STX. This program rewrites the version tag of the file
     specified on the command line. It fixes the problem of
     uninitialized garbage that is written at the end of the version
     tag string. This garbage can cause some programs (notably Mac
     Collage) to choke.

   * read hdf files into Iris Explorer

     The NCSA PATHFINDER project contains a module ReadDF which reads
     HDF files into IRIS Explorer. The module is available on the NCSA
     anonymous ftp server.

     The module can now read from either HDF or netCDF files since it
     uses the NCSA MFHDF library. It runs on IRIX version 4 and
     Explorer 2.0.
   ...
   * hdfinfo
     ...
     Hdfinfo.c was written by William Sherman of NCSA. It compiles
     with the HDF3.2 library. The package is in subdirectory hdfinfo/.

   * HDF 24 bit image animator

     Andrea Leone, Centre for Advanced Studies, R&D Sardinia, has
     contributed two programs, hdf24seq and mhdf24seq, which have been
     tested on the SGIs and IBM RS/6000. Both programs visualize and
     animate 24 bit images from HDF files. mhdf24seq allows the user
     to visualize images from more than one hdf file at a time.
