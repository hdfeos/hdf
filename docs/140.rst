...
Important changes include:
   ...
   * The H5free_memory (C) api was added to free memory that was
     allocated by HDF5 API calls. This was added specifically for
     Windows, but may be useful anywhere that the library uses a
     different memory manager than the application.

   * New wrappers were added for C++ (many) and Fortran
     (h5pset_file_image_f and h5pget_file_image_f).

   * Mac OS X 10.9 Mavericks is now supported.

   * Many improvements were added in support of CMake, including the
   ...
   * A few changes were added to the tools (h5diff, h5dump, and
     h5repack).
