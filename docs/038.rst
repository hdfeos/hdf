...
some of the limitations of the HDF4 library, and to address current
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   * Fill values have been added for datasets. For contiguous datasets
     fill value performance may be quite poor since the fill value is
     written to the entire dataset when the dataset is created. This
     will be remedied in a future version. Chunked datasets using fill
     values do not incur any additional overhead. See
     H5Pset_fill_value().
   ...
   * Object names can be removed or changed but objects are never
     actually removed from the file yet. See H5Gunlink() and
     H5Gmove().

   * A tuning mechanism for B-trees has been added to insure that
     sequential writes to chunked datasets use less overhead. See
     H5Pset_btree_ratios().
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   * The error handling API ``H5E`` has been completed. By default,
     ...
     on the standard error stream. The H5Eset_auto() controls the
     automatic printing and H5E_BEGIN_TRY/H5E_END_TRY macros can
     temporarily disable the automatic error printing.
