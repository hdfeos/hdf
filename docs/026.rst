...
This official release is based on the Java Development Kit JDK 1.1.
...
Following are the new features and changes in JHV for this release:
   ...
   * It is based on JDK 1.1, and uses new features of the Java
     environment that were not available in JDK 1.02.

   * It uses and requires the JHI, described below.

   * Palette manipulation has been made into a separate package
     (``ncsa.hdf.palette``), which may be used independently of the
     JHV or HDF library.

   * There are many internal changes and bug fixes.
...
package "wraps around" the standard HDF 4.1r1 library, which is
called from Java through ``native`` methods.
