...
The HDF5-1.8.11 release is now available, and can be downloaded from
...
HDF5-1.8.11 is a minor release, with several important new features
and changes, including:

   * The library can now load filter libraries dynamically during
     runtime. Users can set a search path through an environment
     variable (HDF5_PLUGIN_PATH) and call H5Pset_filter to enable the
     dynamic filter. For further information see:

     | http://www.hdfgroup.org/HDF5/doc/Advanced/DynamicallyLoadedFilters/HDF5DynamicallyLoadedFilters.pdf

   * Several new API functions were added:
     ...
     * Many Fortran APIs were added:
       ...
       h5oget_comment_by_name_f, h5oincr_refcount_f,
       h5oopen_by_idx_f, h5oset_comment_f, h5oset_comment_by_name_f

       F2003: h5oget_info_f, h5oget_info_by_idx_f,
       h5ovisit_by_name_f

     * A new High Level API, H5DOwrite_chunk, writes a data chunk
       directly into a file, bypassing hyperslab selection, data
       conversion, and the filter pipeline. For more information see:
   ...
   * Support was added for Sun OS 5.11 (32-bit, 64-bit) with the Sun
     C/C++ 5.12 compiler and the Sun Fortran 95 8.6 compiler.

   * Support was added for Visual Studio 2012 with Intel Fortran 13 on
     Windows 7.

     We are working on porting HDF5 to Windows 8. If you are
     interested in using HDF5 with Windows 8, please contact The HDF
     Group Helpdesk at help@hdfgroup.org.

   * The CMake build on Windows will no longer use legacy naming for
     the libraries. The "dll" tag will no longer be added to the name
     of \*.lib and \*.dll.


   * Numerous changes were made to the tools, including a performance
     improvement to h5diff, when comparing HDF5 files with many
     attributes.
